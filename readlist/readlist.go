...
func (l *ReadList) report() {
	...
	for {
		...
		case <-timer.C:
			var (
				avgComp     time.Duration
				avgPureComp time.Duration
				avgCompSize int64
			)
			if l.compN.Load() > 0 {
			...
			fmt.Println(
				"comp tasks", l.compN.Load(), "compAvg", avgComp, "compPure", avgPureComp, "compSize", avgCompSize,
				"read tasks", l.readN.Load(), "readAvg", avgRead, "readPure", avgPureRead, "readSize", avgReadSize,
			)
			l.readN.Store(0)
