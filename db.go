...
func (d *DB) Get(key []byte) ([]byte, io.Closer, error) {
	v, closer, _, _, _, _, _, err := d.getInternal(key, nil /* batch */, nil /* snapshot */)
	return v, closer, err
}

func (d *DB) GetWithStats(key []byte) ([]byte, io.Closer, uint64, uint64, uint64, time.Duration, []time.Duration, error) {
...
func (d *DB) getInternal(key []byte, b *Batch, s *Snapshot) ([]byte, io.Closer, uint64, uint64, uint64, time.Duration, []time.Duration, error) {
	...
	*i = Iterator{
	...
	if !i.First() {
		...
		return nil, nil, 0, 0, 0, 0, nil, ErrNotFound
	}
	return i.Value(), i, get.iOpts.stats.BlockBytes, get.iOpts.stats.BlockBytesInCache, get.iOpts.stats.BlockReadCount, get.iOpts.stats.BlockReadDuration, get.iOpts.stats.Durations, nil
}
